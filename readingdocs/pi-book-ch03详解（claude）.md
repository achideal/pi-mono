...
#### 4.1b AgentToolCall — 工具调用内容块的提取类型
...
这是一个从 `AssistantMessage.content` 数组元素中提取 `{ type: "toolCall" }` 变体的实用类型。它避免了在 agent-core 层重复定义 `ToolCall` 接口 —— 直接从 pi-ai 的 `AssistantMessage` 推导出来。

这个类型在整个 agent-core 层被广泛使用：`BeforeToolCallContext.toolCall`、`AfterToolCallContext.toolCall`、`prepareToolCall` 的参数、以及 `executeToolCalls` 的内部逻辑都依赖它。
...
export interface BeforeToolCallContext {
  ...
  toolCall: AgentToolCall;             // 来自 assistantMessage.content 的原始工具调用块
  args: unknown;                       // 已通过目标工具 schema 验证的参数
  context: AgentContext;               // 工具调用被 prepare 时的 agent 上下文快照
}
```

逐字段说明：

- `assistantMessage`：请求此工具调用的 assistant 消息。一条 assistant 消息可能同时包含多个 toolCall，钩子通过 `toolCall` 字段区分具体是哪个
- `toolCall`：原始的工具调用块，直接来自 `assistantMessage.content` 数组
- `args`：**已通过 schema 验证**的工具参数。虽然类型是 `unknown`，但实际值已经符合目标工具的 TypeBox schema。钩子可以就地修改这个值，修改后 **不会重新进行 schema 验证**
- `context`：**工具调用被 prepare 时的**当前 agent 上下文快照。注意在 parallel 模式下多个 prepare 是串行的，所以每个钩子看到的 context 可能略有不同
...
export interface AfterToolCallContext {
  ...
  toolCall: AgentToolCall;             // 来自 assistantMessage.content 的原始工具调用块
  args: unknown;                       // 已通过目标工具 schema 验证的参数
  result: AgentToolResult<any>;        // afterToolCall 覆盖生效之前的原始执行结果
  isError: boolean;                    // 当前执行结果是否被视为错误
  context: AgentContext;               // 工具调用被 finalize 时的 agent 上下文快照
}
```

与 `BeforeToolCallContext` 相比，额外的字段蕴含着关键的时序语义：

- `result`：拿到的是**原始执行结果**（`afterToolCall` 覆盖生效之前）。如果有多个 extension 都注册了 `afterToolCall`，后注册的钩子看到的是原始结果还是前一个钩子修改后的结果，取决于具体实现
- `isError`：注意"当前被视为错误"的措辞 —— 这个标志可能被前序逻辑（如工具执行时的异常捕获）设置过
- `context`：与 `BeforeToolCallContext` 的 prepare 时间点不同，这里是 **finalize 时间点**。在 parallel 模式下，finalize 阶段在所有工具并发执行完成后才串行进行
...
职责：在每次 LLM 调用前，将 `AgentMessage[]` 转换为 LLM 兼容的 `Message[]`。每个 `AgentMessage` 必须被转换为 `UserMessage`、`AssistantMessage` 或 `ToolResultMessage`；无法转换的消息（如 UI-only 的通知、状态消息）应被过滤掉。

**契约**：不能抛出异常或返回被拒绝的 Promise，应返回安全的回退值。源码特别警告：**如果 `convertToLlm` 抛出异常，循环引擎不会正常发出 `agent_end` 事件**，而是直接崩溃。这意味着 UI 层可能看不到正常的生命周期事件。因此实现时必须用 try/catch 包裹所有可能出错的逻辑。
...
在 `convertToLlm` 之前应用的可选变换。适用于需要在 `AgentMessage` 层面操作的场景：
- 上下文窗口管理（裁剪过旧的消息）
- 从外部来源注入上下文

**契约**：不能抛出异常。出错时应返回原始消息数组或其他安全回退值。
...
在每次 LLM 调用时动态解析 API 密钥。适用于短期 OAuth token（如 GitHub Copilot）在长时间工具执行过程中可能过期的场景。

**契约**：不能抛出异常。无可用密钥时返回 `undefined`。

注意"每次 LLM 调用时" —— 这个函数不是只在 agent 启动时执行一次，而是每轮都会重新调用。这对于 OAuth token 刷新至关重要。

##### `getSteeringMessages`（可选）

```typescript
getSteeringMessages?: () => Promise<AgentMessage[]>;
```

返回需要在运行中途注入对话的"转向"消息。在当前 assistant 轮的所有工具调用执行完成后调用。如果返回了消息，它们会在下一次 LLM 调用前被加入上下文。**当前 assistant 消息的工具调用不会被跳过** —— 即使有 steering 消息等待注入，当前轮的所有工具调用仍然会完整执行。

**契约**：不能抛出异常。无消息时返回 `[]`。

##### `getFollowUpMessages`（可选）

```typescript
getFollowUpMessages?: () => Promise<AgentMessage[]>;
```

返回在 agent 本应停止后需要继续处理的"后续"消息。仅在没有更多工具调用**且**没有 steering 消息时才会被调用。如果返回了消息，agent 会将它们加入上下文并继续新一轮对话。

**契约**：不能抛出异常。无消息时返回 `[]`。

注意调用时机的隐含优先级：只有 steering 队列清空后才会检查 followUp 队列，即 steering > followUp。
...
工具执行模式。`"sequential"` 逐个执行；`"parallel"` 串行 preflight 后并发执行。默认 `"parallel"`。
...
源码注释中的关键时序信息：
- `beforeToolCall`：在参数 schema 验证**之后**、工具实际执行**之前**调用
- `afterToolCall`：在工具执行完成**之后**、`tool_execution_end` 事件发出**之前**调用
- 两者都接收 agent 的 abort signal，且**钩子自身负责检查和处理中止信号** —— 循环引擎不会替钩子做这件事
...
- `getSteeringMessages` 在 **每轮工具执行完成后** 调用 —— 用于"agent 正在工作时，用户给当前任务追加了新指令"的场景。当前轮的工具调用不会被跳过
- `getFollowUpMessages` 在 **agent 即将停止时（无更多工具调用且无 steering 消息）** 调用 —— 用于"agent 完成后，还有需要继续处理的消息"的场景
- 隐含优先级：steering > followUp。只有 steering 队列清空后才会检查 followUp
- **所有回调共享的契约**：不能抛出异常或返回被拒绝的 Promise（`must not throw or reject`）。这保证了循环引擎的健壮性
...
export interface AgentState {
  systemPrompt: string;              // 每次模型请求都会带上的 system prompt
  model: Model<any>;                 // 未来轮次使用的活跃模型
  thinkingLevel: ThinkingLevel;      // 未来轮次请求的推理级别
  set tools(tools: AgentTool<any>[]);  // 赋值时会浅拷贝数组
  get tools(): AgentTool<any>[];
  set messages(messages: AgentMessage[]);  // 赋值时会浅拷贝数组
  get messages(): AgentMessage[];
  readonly isStreaming: boolean;      // agent 处理中为 true，直到 agent_end 监听器全部 settle
  readonly streamingMessage?: AgentMessage;  // 当前流式响应的部分消息（如有）
  readonly pendingToolCalls: ReadonlySet<string>;  // 正在执行的工具调用 ID 集合
  readonly errorMessage?: string;    // 最近一次失败/中止轮的错误信息（如有）
}
```

源码注释揭示了每个字段的精确语义：

- **`systemPrompt`**：每次 LLM 调用都会带上，而不是只在首轮发送
- **`model`**：用于"未来轮次"的活跃模型 —— 切换模型后，当前正在进行的 turn 不受影响，新模型从下一个 turn 开始生效
- **`thinkingLevel`**："未来轮次请求的推理级别" —— 同理，对 future turns 生效
- **`tools`/`messages`**：赋值时会**浅拷贝**顶层数组（`.slice()`），防止外部代码持有内部数组引用并意外修改。但数组元素本身不会被深拷贝，修改某个消息对象的属性仍然会影响内部状态
- **`isStreaming`**：在 `agent_end` 监听器全部 settle 之后才变为 `false`。这意味着监听器可以在 `agent_end` 回调中安全地认为 agent 仍在"运行中"，适合执行清理操作
- **`streamingMessage`**：只在流式响应期间有值，用于 UI 实时渲染部分消息
- **`pendingToolCalls`**：`ReadonlySet<string>` 类型，外部不能修改。存储的是 tool call id 而非工具名，因为同一个工具可能被同时调用多次
- **`errorMessage`**：只保留"最近一次"失败或中止轮的错误信息，不是累积的

#### 4.8 AgentToolResult 和 AgentTool — 工具的运行时接口

```typescript
export interface AgentToolResult<T> {
  content: (TextContent | ImageContent)[];  // 返回给模型的文本或图片内容
  details: T;                               // 任意结构化详情，仅用于日志和 UI 渲染
}
...
源码注释强调了 `content` 和 `details` 的分工：`content` 是"返回给模型的内容"（会进入 LLM 上下文），`details` 是"用于日志或 UI 渲染的任意结构化详情"（不会发送给 LLM）。`AgentToolUpdateCallback` 被源码描述为"工具流式推送部分执行更新的回调"。

```typescript
export interface AgentTool<TParameters extends TSchema, TDetails = any>
  extends Tool<TParameters> {
  label: string;              // UI 显示的人类友好标签
  prepareArguments?: (args: unknown) => Static<TParameters>;  // schema 验证前的参数兼容性 shim
  execute: (                  // 执行工具调用。失败时应抛出异常，而非在 content 中编码错误
    toolCallId: string,
...
export interface AgentContext {
  systemPrompt: string;        // 随请求一起发送的 system prompt
  messages: AgentMessage[];    // 模型可见的对话记录
  tools?: AgentTool<any>[];   // 本次运行可用的工具
}
```

源码注释精确描述了每个字段的角色：`systemPrompt` 是"随请求一起发送的 system prompt"，`messages` 是"模型可见的对话记录"（transcript visible to the model），`tools` 是"本次运行可用的工具"。
...
/**
 * Agent 发出的 UI 更新事件。
 * agent_end 是一次运行中发出的最后一个事件，但 awaited 的 Agent.subscribe() 监听器
 * 仍是运行 settlement 的一部分。agent 只有在这些监听器完成后才变为 idle。
 */
export type AgentEvent =
  // ── Agent 生命周期 ──
  | { type: "agent_start" }
  | { type: "agent_end"; messages: AgentMessage[] }
  // ── Turn 生命周期 ── 一个 turn = 一次 assistant 响应 + 其触发的所有工具调用/结果
  | { type: "turn_start" }
  | { type: "turn_end"; message: AgentMessage; toolResults: ToolResultMessage[] }
  // ── Message 生命周期 ── 对 user、assistant、toolResult 消息均会发出
  | { type: "message_start"; message: AgentMessage }
  // message_update 仅在 assistant 消息流式传输期间发出
...
- **Turn 的定义**：一个 turn 等于"一次 assistant 响应 + 其触发的所有工具调用/结果"（a turn is one assistant response + any tool calls/results）
- **`message_start`/`message_end` 的覆盖范围**：对 user、assistant、toolResult 消息**均会发出**，不仅限于 assistant 消息
- **`message_update` 的作用域**：**仅在 assistant 消息流式传输期间发出**（Only emitted for assistant messages during streaming）—— user 消息和 toolResult 消息不会触发 `message_update`
- **`agent_end` 与 idle 的关系**：`agent_end` 是最后一个事件，但 agent 并不会立刻变为 idle —— 它会等待所有 `Agent.subscribe()` 监听器 settle 后才进入 idle 状态
...
**注意**：`agent_end` 虽然是最后一个事件，但 awaited 的 `Agent.subscribe()` 监听器仍是运行 settlement 的一部分。agent 只有在这些监听器完成后才变为 idle。
...
```
`processEvents`被作为 `emit` 回调传给 agent-loop。每当 loop 产生一个事件，它做两件事：

1. **更新 `_state`**（reducer）— 根据事件类型修改 `streamingMessage`、`messages`、`pendingToolCalls`、`errorMessage` 等字段
2. **顺序 await 所有 listener** — 通知外部订阅者（UI 渲染、日志、状态持久化等）

它是 agent-loop（纯逻辑）和外部世界（状态 + 订阅者）之间的桥梁。

这个方法是 Agent 状态管理的核心 —— 它像一个 **reducer**（接收当前状态和一个动作/事件，根据事件类型决定如何更新状态） 一样，根据事件更新内部状态：
